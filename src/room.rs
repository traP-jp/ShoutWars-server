...
pub struct Room {
    ...
    pub info: Value,
    /// 部屋主が送ってきた次の `info`。レコードの締め切り時に反映する。
...
impl Room {
    ...
    /// 部屋そのものの期限。ロビーは作成から、ゲームは開始から数える。
    ...
    pub fn close(&mut self) {
        ...
        // 部屋情報の差し替えも締め切りに合わせる。通知イベントと同じ境界で切り替わる。
    ...
    /// クライアントの申告と突き合わせる。一度でも食い違えば以後は立ったまま。
    ///
    /// ゲームの状態そのものをハッシュして突き合わせる方式は採らない。
    /// 何を対象に含めるかを全クライアントで揃え続ける必要があり、揃っていなければ誤検出する。
    /// 件数の比較ならゲームの内容に依存しない。増分ハッシュ (XOR) も適さない。自己逆元であるため、
    ...
    /// 応答が途絶えたユーザーを部屋から除く。
    ///
    /// 返すのは無効になったセッション。呼び出し側が登録簿から消す。期間はレコードの保持数と揃える。
    /// 仕様の 10 秒はどちらも同じ値であり、一方だけを短くすると、
    /// 部屋には残っているのに追いつけないユーザーが生じる。
