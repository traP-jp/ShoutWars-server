...
//!
//! 個々の規則を確かめるテストとは狙いが違う。レコードの締め切り、不在と復帰、
//! カーソルの進行、`applied` の照合は、どれも単体では正しく見えて、絡み合った
//! ときに初めて壊れる。
...
/// うちに次を送ることになり、403 already_synced が返る。仕様の「`tick_ms` 周期で
/// 送信する」は、クライアントが守るべき制約である。
///
/// 幅を持たせてあるのは、締め切りに間に合う回と間に合わない回を混ぜるため。
/// 不在の判定と、不在だった人が次のレコードから戻る経路を通す。
